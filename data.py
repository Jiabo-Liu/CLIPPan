...
class Datain_stx(data.Dataset):

    def __init__(self, root, ratio):  # root: data in path   初始化路径和其他参数
    ...
    def __getitem__(self, item):  # 加载图像并进行处理
        ...
        # f.close()
        img_ms = scio.loadmat(img_path_ms)['imgMS']  # <class 'numpy.ndarray'>
        img_path_pan = self.imgs_pan[item]
        img_pan = scio.loadmat(img_path_pan)['imgPAN']
        # print(img_ms.shape)
        # print(img_pan.shape)
        # print(type(img_pan))
        ...
        '''data format change'''# 格式转换,改为浮点数类型tensor
        ...
        '''channels change position'''# 通道顺序转换
...
def Gaussian_downsample(z, fft_B, sf=4, s0=1):
    if z.ndim == 2:
        M, N = z.shape
        L = 1
        x = np.zeros((round(M / sf), round(N / sf)))
        Hz = np.real(np.fft.ifft2(np.fft.fft2(z[:, :]) * fft_B))
        t = Hz[s0::sf, s0::sf]
        x[:, :] = t
    else:
        M, N, L = z.shape
        x = np.zeros((round(M / sf), round(N / sf), L))

        for i in range(L):
            Hz = np.real(np.fft.ifft2(np.fft.fft2(z[:, :, i]) * fft_B))
            t = Hz[s0::sf, s0::sf]
            x[:, :, i] = t
    return x
...
class Datain_B(data.Dataset):
    def __init__(self, root, ratio, fft_B_1024, fft_B_256):  # root: data in path   初始化路径和其他参数
        self.ms_save_path = os.path.join(root, 'ms')
        self.pan_save_path = os.path.join(root, 'pan')
        ...
        # print('self.imgs_pan : ', self.imgs_pan)
        self.ratio = ratio
        self.fft_B_1024 = fft_B_1024
        self.fft_B_256 = fft_B_256
    def __getitem__(self, item):  # 加载图像并进行处理
        img_path_ms = self.imgs_ms[item]  # the path of the item-th img
        # print('img_path_ms: ', img_path_ms)
        ...
        #     f.write('img_path_ms: ' + img_path_ms + '\n')
        # f.close()
        img_ms = scio.loadmat(img_path_ms)['ms']  # <class 'numpy.ndarray'>  ms   imgMS
        img_path_pan = self.imgs_pan[item]
        img_pan = scio.loadmat(img_path_pan)['ms']  # ms  imgPAN
        # print(img_ms.shape)
        # print(img_pan.shape)
        # print(type(img_pan))
        ...
        pan_np = Gaussian_downsample(img_pan, self.fft_B_1024, 4, 1).reshape(ms_size, ms_size, 1)
        ms_np = Gaussian_downsample(img_ms, self.fft_B_256, 4, 1)
        '''
        shape
        (256, 256, 4)
        (256, 256, 1)
...
def augment(ref_np, pan_np, ms_np):  #  图像增强。
    # horizontal flip OR rotate
    x = random.random()
    if x < 0.5:
        ref_np = ref_np[:, ::-1, :] #水平翻转
        pan_np = pan_np[:, ::-1, :]
        ms_np = ms_np[:, ::-1, :]
        ref_np = ref_np[::-1, :, :] # 垂直翻转
        pan_np = pan_np[::-1, :, :]
        ms_np = ms_np[::-1, :, :]

        ref_np = ref_np.transpose(1, 0, 2) # 顺时针旋转90度
        pan_np = pan_np.transpose(1, 0, 2)
        ms_np = ms_np.transpose(1, 0, 2)

    return ref_np, pan_np, ms_np

class Datain(data.Dataset):
    def __init__(self, root, ratio, train=None):  # root: data in path   初始化路径和其他参数
        self.ms_save_path = os.path.join(root, 'ms')
        self.pan_save_path = os.path.join(root, 'pan')
    ...
    def __getitem__(self, item):  # 加载图像并进行处理
        if item >=len(self.imgs_ms):
            item =item-len(self.imgs_ms)
        else:
            item = item
        img_path_ms = self.imgs_ms[item]  # the path of the item-th img
        # print('img_path_ms: ', img_path_ms)
        ...
        img_ms = scio.loadmat(img_path_ms)['ms']  # <class 'numpy.ndarray'>  ms   imgMS
        img_path_pan = self.imgs_pan[item]
        img_pan = scio.loadmat(img_path_pan)['pan']  # ms  imgPAN
        # print(img_ms.shape)
        # print(img_pan.shape)
        # print(type(img_pan))
        ...
        # 数据增强
        # if self.train:
        #     ref_np, pan_np, ms_np = augment(ref_np, pan_np, ms_np)
        #     ref_np = ref_np.copy()
        #     pan_np = pan_np.copy()
        #     ms_np = ms_np.copy()
    ...
    def __len__(self):  # 返回数据集中图像的数量
        if self.train:
            return len(self.imgs_ms) * self.repeat  # 训练集，则返回HR图像的数量乘以repeat参数的值
        else:
            return len(self.imgs_ms)

class Datain_real_data(data.Dataset):
    def __init__(self, root, ratio, train=None):  # root: data in path   初始化路径和其他参数
        self.ms_save_path = os.path.join(root, 'ms')
        self.pan_save_path = os.path.join(root, 'pan')
...
class Datain_real_data_des(data.Dataset):
    def __init__(self, root, ratio, train=None):  # root: data in path   初始化路径和其他参数
        self.ms_save_path = os.path.join(root, 'ms')
        self.pan_save_path = os.path.join(root, 'pan')
    ...
    def __getitem__(self, item):  # 加载图像并进行处理
        if item >=len(self.imgs_ms):
            ...
            item = item
        text_keys = ['pan_description', 'image_description', 'text_description',
        ...
        #     f.write('img_path_ms: ' + img_path_ms + '\n')
        # f.close()
        ms_mat = scio.loadmat(img_path_ms)  # <class 'numpy.ndarray'>  ms   imgMS
        img_ms = ms_mat['ms']
        for key in text_keys:
            if key in ms_mat:
                ms_description = ms_mat[key]
        img_path_pan = self.imgs_pan[item]
        pan_mat = scio.loadmat(img_path_pan)  # ms  imgPAN
        img_pan = pan_mat['pan']
        for key in text_keys:
            if key in pan_mat:
                pan_description = pan_mat[key]
        # print(img_ms.shape)
        # print(img_pan.shape)
        # print(type(img_pan))
        ...
        torch.Size([4, 64, 64])
        '''
        return pan, ms, ms_description, pan_description  # 返回图片对应的tensor及其标签
