...
class Q2n(nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        # 2. 计算分块参数
        stepx = max(1, math.ceil(dim1 / self.Q_shift))
        stepy = max(1, math.ceil(dim2 / self.Q_shift))

        # 3. 安全计算填充尺寸
        est1 = max(0, (stepx - 1) * self.Q_shift + self.Q_block_size - dim1)
        est2 = max(0, (stepy - 1) * self.Q_shift + self.Q_block_size - dim2)

        # 4. 安全的反射填充
        if est1 > 0 or est2 > 0:
            # 确保填充尺寸小于输入尺寸
            est1 = min(est1, dim1 - 1)
            est2 = min(est2, dim2 - 1)
        ...
        for j in range(stepx):
            ...
            for i in range(stepy):
                block_start_x = j * self.Q_shift
                block_end_x = min(block_start_x + self.Q_block_size, dim1)
                block_start_y = i * self.Q_shift
                block_end_y = min(block_start_y + self.Q_block_size, dim2)

                o = q_index(
                    labels[:, :, block_start_x:block_end_x, block_start_y:block_end_y],
                    outputs[:, :, block_start_x:block_end_x, block_start_y:block_end_y],
                    min(self.Q_block_size, block_end_x - block_start_x, block_end_y - block_start_y),
                    self.device
                )
