...
class AttnFuseMain(nn.Module):
    def __init__(
        ...
        self.nihe = nn.Conv2d(lms_dim, 1, kernel_size=1)
        self.patch_merge = patch_merge

    def forward(self, lms, pan):
        ms = F.interpolate(lms, scale_factor=4, mode="bicubic")
        reused_attn, refined_lms = self.attn(ms, pan)
        pre_hp = self.pre_hp(ms, pan)
...
if __name__ == '__main__':
    ...
    net = AttnFuseMain(pan_dim=1, lms_dim=8, attn_dim=64, hp_dim=64, n_stage=3)
