...
def SSIM_numpy(x_true, x_pred, data_range, sewar=False):
    ...
    return structural_similarity(x_true, x_pred, data_range=data_range, multichannel=True,channel_axis=2)
...
def calc_psnr_numpy(img_tgt, img_fus):
...
def QNR(fusion,ms,pan,alpha = 1,beta = 1):
    ...
    D_lambda_index = D_lambda_numpy(ms,fusion)
    D_s_index = D_s_numpy(ms,pan,fusion)
